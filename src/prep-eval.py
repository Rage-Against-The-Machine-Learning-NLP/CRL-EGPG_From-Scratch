...
from src.utils import Specials, get_bert_tokenizer, pkl_load


def evaluate(config_file: str, model_file: str) -> None:
    ...
    seq2seq = Seq2Seq(config, device=device)
    seq2seq.load_state_dict(torch.load(os.path.join(model_file, "seq2seq.pkl")))
    seq2seq.eval()

    style_extractor = StyleExtractor()
    style_extractor.load_state_dict(
        torch.load(os.path.join(model_file, "style_extractor.pkl"))
    )
    ...
    with torch.no_grad():
        for idx, (
            ...
            for similar_sent_idx in sim[idx]:
                ...
                SENT_LEN = min(config.max_len + 2, len(bert_sent_opt))

                bert_sim_sent = torch.zeros(config.max_len + 2, dtype=torch.long)
    ...
    generated_file = os.path.join(model_file, "trg_gen.txt")
    ...
    exemplar_file = os.path.join(model_file, "trg_exmp.txt")
